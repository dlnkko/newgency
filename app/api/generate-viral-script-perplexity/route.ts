...
import { generateTextWithSonarPro } from '@/lib/perplexity';
...
export async function POST(request: NextRequest) {
  try {
    // Check rate limit
    const rateLimitResult = await checkRateLimit('generateViralScriptPerplexity', request);
    ...
    // Transform transcript using Perplexity Sonar Pro
...
Provide ONLY the creatively transformed script as a single continuous paragraph. It should be a fresh, improved version that captures the original's energy and structure but is completely rewritten with creative, unique language focused on the user's product. No headers, no explanations, no line breaks - just the script text flowing naturally in one paragraph.`;
    ...
    try {
      const systemPrompt = `You are an expert creative writer specializing in viral marketing scripts. Your responses are always creative, engaging, and perfectly formatted as requested.`;
      
      scriptText = await generateTextWithSonarPro(
        transformationPrompt,
        systemPrompt,
        {
          model: 'sonar-pro',
          temperature: 0.7, // Un poco más creativo para scripts virales
          max_tokens: 4096,
        }
      );
      ...
      console.error('Error calling Perplexity Sonar Pro:', perplexityError);
      return NextResponse.json(
        { 
          error: 'Error transforming script with Perplexity Sonar Pro',
          details: perplexityError.message || 'Could not process request with AI'
        },
        { status: 500 }
      );
    }
    ...
    console.error('Error generating viral script with Perplexity:', error);
    
    if (error.message?.includes('API key')) {
      return NextResponse.json(
        { error: 'API configuration error. Please check your PERPLEXITY_API_KEY environment variable.' },
        { status: 500 }
      );
    }
