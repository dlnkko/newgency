...
2. **Specify what to copy** from the reference image based on user selection:${copyCameraAngle ? '\n   - **MANDATORY - Copy Camera Angle**: Explicitly state that the camera angle, perspective, framing, and composition must match EXACTLY the attached reference image' : ''}${copyLighting ? '\n   - **MANDATORY - Copy Lighting**: Explicitly state that the lighting style, direction, intensity, color temperature, shadows, and highlights must match EXACTLY the attached reference image' : ''}
